  ...
  $.fn.extend({
          ...
          if ($(this).parent().find('ul').length !== 0) {
            ...
            if (
              ...
              $(this)
                .parent()
                .find('ul:first')
                .slideUp(opts.speed + 100, opts.animate, function () {
                  $(this)
                    .parent('li')
                    .removeClass('open')
                    .find('a:first')
                  ...
                  if (myapp_config.debugState) console.log('nav item closed');
                });
            } else {
              $(this)
                .parent()
                ...
                .slideDown(opts.speed, opts.animate, function () {
                  ...
                  if (myapp_config.debugState) console.log('nav item opened');
                });
            }
          }
        });
    ...
    navigationDestroy: function () {
      ...
      if (self.hasClass(myapp_config.navInitalized)) {
        ...
        if (myapp_config.debugState) console.log(self.get(0) + ' destroyed');
      } else {
...
(function ($) {
  ...
  function Plugin(element, options) {
    ...
    function init() {
      ...
      var getListWidth = $.map($el.children('li:not(.nav-title)'), function (
          val,
        ) {
        ...
        _getValues = function () {
          ...
          contentWidth = $.map($el.children('li:not(.nav-title)'), function (
            val,
          ) {
            return $(val).outerWidth(true);
          }).reduce(function (a, b) {
...
var initApp = (function (app) {
  ...
  app.resetSettings = function () {
    ...
    myapp_config.root_.removeClass(function (index, className) {
      return (
        className.match(/(^|\s)(nav-|header-|footer-|mod-|display-)\S+/g) || []
      ).join(' ');
    });
    ...
    if (typeof bootbox != 'undefined') {
      bootbox.confirm({
        title:
          ...
          " fa-exclamation-triangle text-warning mr-2'></i> You are about to reset all of your localStorage settings",
        message:
    ...
    switch (true) {
      case myapp_config.root_.hasClass('nav-function-fixed') &&
        ...
        if (typeof $.fn.slimScroll !== 'undefined') {
          ...
          if (myapp_config.debugState) console.log('slimScroll created');
        } else {
      ...
      case myapp_config.navAnchor.parent().hasClass('slimScrollDiv') &&
        ...
        if (events) jQuery._removeData(myapp_config.navAnchor[0], 'events');

        if (myapp_config.debugState) console.log('slimScroll destroyed');
  ...
  app.domReadyMisc = function () {
    ...
    $(document).on(
      'click',
      '.dropdown-menu:not(.js-auto-close):not(.note-dropdown-menu)',
      function (e) {
        e.stopPropagation();
      },
    );
    ...
    myapp_config.root_.on('click touchend', '[data-action]', function (e) {
      ...
      switch (true) {
        ...
        case actiontype === 'panel-fullscreen':
          ...
          if (myapp_config.debugState)
            console.log(
              'panel id:' + selectedPanel.attr('id') + ' | action: fullscreen',
            );
        ...
        case actiontype === 'panel-close':
          ...
          var killPanel = function () {
            selectedPanel.fadeOut(500, function () {
              ...
              if (myapp_config.debugState)
                console.log(
                  'panel id:' + selectedPanel.attr('id') + ' | action: removed',
                );
            });
          };
...
var layouts = (function (setlayout) {
  setlayout.errorMessage = function (layout) {
    console.log(
      "('" + layout + "')" + " is not a valid entry, enter 'on' or 'off'",
    );
  };
      ...
      default:
        console.log(
          "('" +
            layout +
            "')" +
            " is not a valid entry, enter 'default', 'light', or 'dark'",
        );
        break;
    }
  };
...
window['myapp_config'] = myapp_config;
window['initApp'] = initApp;

initApp.updateConfigs = () => {
  ...
  myapp_config.navFilterInput = $('#nav_filter_input');
...
initApp.domReadyMisc = () => {
  ...
  /* Give modal backdrop an extra class to make it customizable */
  $('.modal-backdrop-transparent').unbind('show.bs.modal');
  $('.modal-backdrop-transparent').on('show.bs.modal', function (e) {
    setTimeout(function () {
      $('.modal-backdrop').addClass('modal-backdrop-transparent');
    });
  ...
  if (
    ...
    $('.custom-scroll:not(.disable-slimscroll) >:first-child').unbind(
      'slimscroll',
    );
    $('.custom-scroll:not(.disable-slimscroll) >:first-child').slimscroll({
      height: $(this).data('scrollHeight') || '100%',
   ...
   * doc: close dropdowns on click outside hit area
   **/
  $('.dropdown-menu:not(.js-auto-close):not(.note-dropdown-menu)').unbind(
    'click',
  );
  $(document).on(
    'click',
  ...
  myapp_config.root_.unbind('click touchend');
  myapp_config.root_.on('click touchend', '[data-action]', function (e) {
    var actiontype = $(this).data('action');
    ...
    switch (true) {
      ...
      case actiontype === 'panel-collapse':
        ...
        selectedPanel.unbind('show.bs.collapse');
        selectedPanel
          .children('.panel-container')
          .collapse('toggle')
          .on('show.bs.collapse', function () {
          ...
          .on('hidden.bs.collapse', function () {
            ...
            if (myapp_config.debugState)
              console.log(
                'panel id:' + selectedPanel.attr('id') + ' | action: collapsed',
              );
          });
